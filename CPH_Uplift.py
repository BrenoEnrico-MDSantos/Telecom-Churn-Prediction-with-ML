...
df = pd.DataFrame({
    ...
    'Tenure_Months': np.random.randint(1, 72, n_total),
    'Monthly_Charges': np.random.uniform(30, 250, n_total),
    'Total_Revenue': np.random.uniform(100, 5000, n_total)
})
...
# =========================================================================
# CLASSE MOCK ATUALIZADA (Simula exatamente a estrutura do lifelines)
# =========================================================================
class MockCPH:
    def __init__(self):
        self.params_ = pd.Series({
            ...
            'treatment_2': -0.65,
            'treatment_3': -0.40,
            'decaimento_Premium': 0.015,
            'decaimento_Standard': 0.035,
            'decaimento_Budget': 0.060,
            'decaimento_Retention_Risk': 0.080
        })
cph_final = MockCPH()
# =========================================================================

# 2. CÁLCULO DO RISCO BASE ESTÁTICO (Filtra apenas colunas preditivas do CPH)
colunas_cph_base = [col for col in df_encoded.columns if col in cph_final.params_.index]
log_hazard_base = df_encoded[colunas_cph_base].dot(cph_final.params_[colunas_cph_base])
df['Baseline_Churn_Prob'] = 1 / (1 + np.exp(-log_hazard_base))

df_piloto, df_holdout = train_test_split(df, test_size=0.5, random_state=42)
df_piloto, df_holdout = df_piloto.copy(), df_holdout.copy()

# 3. RANDOMIZAÇÃO RESTRITA OPERACIONAL (Correção de sintaxe dos dicionários)
cph_elegibilidade = pd.DataFrame({
    0: [1, 1, 1, 1], 
    1: [0, 1, 1, 0], 
    2: [1, 0, 1, 0], 
...
# 4. EXTRAÇÃO AUTOMÁTICA DAS TAXAS DE DECAIMENTO DO MODELO POR CLUSTER
chaves_decaimento = f'decaimento_' + df_piloto['Cluster'].values
decaimento_por_cliente_piloto = np.array([cph_final.params_.get(k, 0.04) for k in chaves_decaimento])
fator_tempo_piloto = np.exp(-decaimento_por_cliente_piloto * df_piloto['Tenure_Months'].values)

# 5. CÁLCULO DO EFEITO DE INTERVENÇÃO 100% DINÂMICO E VETORIZADO
df_piloto_encoded = encoder.transform(df_piloto)
tratamentos_piloto = df_piloto['treatment'].values
...
efeito_charges = np.where(df_piloto['treatment'] == 1, -0.001 * df_piloto['Monthly_Charges'], 0)
efeito_tenure = np.where(df_piloto['treatment'] == 2, -0.003 * df_piloto['Tenure_Months'], 0)

# Multiplicação matricial direta e modulação do risco WoE sem dicionários manuais
efeito_base_campanha = (df_piloto_encoded['Cluster'] * coefs_tratamento) + (df_piloto_encoded['State'] * (coefs_tratamento * 0.5)) + efeito_charges + efeito_tenure
df_piloto['efeito_intervencao'] = (efeito_base_campanha * fator_tempo_piloto) + np.random.normal(0, 0.02, len(df_piloto))

df_piloto['Churn_Prob_Real'] = np.clip(df_piloto['Baseline_Churn_Prob'] + df_piloto['efeito_intervencao'], 0, 1)
df_piloto['Churn_Label'] = np.random.binomial(1, df_piloto['Churn_Prob_Real'])

# 6. TREINAMENTO DO MODELO OPERACIONAL DE UPLIFT (S-LEARNER)
X_piloto = encoder.transform(df_piloto[['Cluster', 'State', 'Tenure_Months', 'Monthly_Charges']])
X_piloto['treatment'] = df_piloto['treatment']
y_piloto = df_piloto['Churn_Label']
...
# 7. ENGENHARIA DE MATRIZ SCENARIO (Correção de sintaxe da lista de estratégias)
df_holdout_encoded = encoder.transform(df_holdout[['Cluster', 'State', 'Tenure_Months', 'Monthly_Charges']])
n_holdout = len(df_holdout_encoded)
lista_estrategias = [0, 1, 2, 3]
...
# 8. TOMADA DE DECISÃO DETERMINÍSTICA VETORIZADA
matriz_uplifts = df_holdout[[f'Uplift_Pacote_{t}' for t in lista_estrategias[1:]]].values
max_uplifts = np.max(matriz_uplifts, axis=1)
indices_max_uplift = np.argmax(matriz_uplifts, axis=1) + 1
...
df_holdout['Melhor_Ação_Automatica'] = np.where(
    max_uplifts <= 0, 
    'Sem Intervenção (Controle)', 
    pd.Series(indices_max_uplift).apply(lambda idx: f'Enviar Pacote {idx}').values
)

# 9. PROFILING DE ENTREGA PARA O NEGÓCIO
tree_profiler = DecisionTreeClassifier(max_depth=3, min_samples_leaf=100, random_state=42)
tree_profiler.fit(df_holdout_encoded, df_holdout['Melhor_Ação_Automatica'])

print("--- Distribuição no Holdout com Decaimento por Cluster ---")
print(df_holdout['Melhor_Ação_Automatica'].value_counts(normalize=True) * 100)
print("\n--- Regras de Profiling Atualizadas ---")
print(export_text(tree_profiler, feature_names=list(df_holdout_encoded.columns)))

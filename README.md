...
Churn prediction and intervention with python, Copilot + Gemini and PowerBI

<div align="center">

<img src="https://github.com/BrenoEnrico-MDSantos/Telecom-Churn-Prediction-with-ML/blob/main/img/TarangTel full.png" alt="Logotype" width="500" style="margin-right:50px;">

<p><em>Meet TarangTel and it's CRM worries</em></p>

</div>

## Background
  *TarangTel*  is a made-up **telecommunication company**, aiming to provide India with quality Phone, Internet and Streaming services. Due to yet uncertain reasons, the clientelle is churning heavily across all states, and chunks of current and potential revenue are being lost by the month. The recently joined customers are in high risk, and analysis were issued to provide direction to the Marketing and Provision team. As such, a diverse subset of clients was provided to foster data-driven decision making, leveraging **Machine Learning concepts and algorithms** alongside **industry-expert AI** to cook modern, efficient solutions to the churn problem.
...
- In the final cust. table (Churn + Stayed + predicted Joined), these columns are now present:
...
| Tech | *Why?* |
|--|--|
| ```StandardScaler``` | After dealing with outliers in numerical data ([IQR/Tukey's Fence Method](https://profkelley.github.io/IQR-Tukeys-Method/) + median replacement), any geometrical algorithm (PCA, MCA, **Factor Analysis of Mixed Data**) expects a variance of 1.0; [MinMax and Robust](https://machinelearningmastery.com/minmax-vs-standard-vs-robust-scaler-which-one-wins-for-skewed-data/) squash variances, therefore importance of data points. |
| ```FAMD``` | **PCA** is applied for categorical types; **MCA** for continuous types; with balanced inertia points, the hybrid matrix of FAMD solves metric incompatibility and prevents type bias. After |
| ```K-Medoids``` | Introducing FAMD principal coordinates into **Euclidean** K-Medoids, mixed types are natively computed and each *k*'s medoid will be an actual data point, unlike **K-Means, K-Modes** and **K-Prototypes**. For bigger datasets, however, $O(n²)$ (quadratic complexity) is unviable, thus lower cost (such as $O(n)$ (linear) tools are needed for scalability. As plotted in the notebook, the **Elbow** test isn't quite telling, so 5 clusters were chosen for business logic and decent scores. |
| ```Smoothed Log Odds Ratio + Lift``` | ***Odds Ratio*** (OR) calculates the probability of a value in the observed subset (Odds) comparing to a control group. ***Log*** is applied to standardize OR to a symmetrical scale, and a constant/*alpha* of 1 (***Laplace Smoothing***) is added to filter irrelevant noise caused by small sampling. ***Lift*** is finally used to visualize observed frequency by expected global mean (e.g. ```Internet_Type: DSL, Lift: 8.1``` = 8.1x more presence of such value when compared to other groups). So, Log Odds Ratio with an added constant means % only for relevant observed events/values; Lift is the multiplicative "translation" to order frequencies and help in acting/profiling |
| ```KMF & COX PH``` | The [Kaplan-Meier Curve](https://www.sthda.com/english/wiki/survival-analysis-basics) displays the survivability of one or multiple groups given a timeline and some observed event (like churning); aside from the plot, milestones such as "80% survival hit" or **median survival** are used for more analysis envolving urgency and action timeframes. [Cox Proportional Hazards model](https://www.sthda.com/english/wiki/cox-proportional-hazards-model)(CPH), on the other hand, expands upon the influence of each feature/**covariate** on the event, culminating in the **Hazard Ratio** of covariates. For campaign simulations, the CPH predicted survival curves are plotted for each strategy , and allow for visual efficacy assessment over survival length. Must be noted that time-dependent, and increasing/decreasing risks **violate** the assumptions needed for standard CPH, which is in use. |
| ```WOEEncoder``` | **Weight of Evidence** (WoE) is a staple for credit scoring and churn modelling. It encodes categorical variables into a numeric indicator of relation to a binary event (1 for churn, 0 for stayed); for example, a **high positive WoE** means that a value inside of a feature relates to **churn tendecy** (perhaps reasons), whereas **high negative WoE** might give insights about **retention**; in the project, it's applied for assessing regional drivers, campaign focus and feature engineering. |
|```XGBoost w/ Stratified K-Fold + Optuna)```| For small/medium datasets, XGBoost outperforms [ensemble algorithms](https://www.geeksforgeeks.org/machine-learning/random-forest-algorithm-in-machine-learning/) and [CatBoost and LightBGM](https://kishanakbari.medium.com/xgboost-vs-catboost-vs-lightgbm-a-guide-to-boosting-algorithms-47d40d944dab), but much tuning is required. After feature engineering, (
[Stratified K-Fold](https://www.geeksforgeeks.org/machine-learning/stratified-k-fold-cross-validation/) ensures adequate sampling across clusters; ***Optuna*** then takes care of hyper-parameter tuning, with the optimal cutoff being the **F1-Score** (balancing *precision* and *recall*). As such, for each classified new customer, the churn risk becomes actionable through labels, being the both medium and high-risk predictions considered urgent. |
